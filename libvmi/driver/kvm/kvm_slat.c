...
// TODO: Maybe we can? Check kvmi_control_ept_view().
...
status_t kvm_switch_view(vmi_instance_t vmi, uint16_t view)
{
    ...
    if (kvm->libkvmi.kvmi_get_ept_view(kvm->kvmi_dom, 0, &current_view)) {
        return VMI_FAILURE;
    }
                        ...
                        addr_t new_gfn)
{
    ...
    if (kvm_switch_view(vmi, slat_idx) == VMI_FAILURE) {
        return VMI_FAILURE;
    }

    for (unsigned int vcpu = 0; vcpu < vmi->num_vcpus; vcpu++) {
        if (kvm->libkvmi.kvmi_change_gfn(kvm->kvmi_dom, vcpu, old_gfn, new_gfn)) {
            return VMI_FAILURE;
        }
    }

    return kvm_switch_view(vmi, current_view);
}
